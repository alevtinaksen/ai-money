...
import { CloseOutlined, PlusOutlined, CheckOutlined, SearchOutlined, CloseCircleFilled } from '@ant-design/icons';
  ...
  const filteredAccounts = useMemo(() => {
    ...
    return accounts.filter((acc) => {
      ...
      return (
        ...
        resolved.cleanName.toLowerCase().includes(q) ||
        (resolved.bank?.name.toLowerCase().includes(q) ?? false) ||
        (resolved.bank?.shortName.toLowerCase().includes(q) ?? false) ||
        (acc.bank_name?.toLowerCase().includes(q) ?? false) ||
    ...
    <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/40 backdrop-blur-sm animate-fade-in">
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-[#1A1B20] rounded-t-[32px] pt-3 pb-8 px-5 shadow-2xl z-10 max-h-[85vh] flex flex-col animate-slide-up transition-colors">
        ...
        <div className="relative my-2">
          <SearchOutlined className="absolute left-3.5 top-3 text-gray-400 text-[15px]" />
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Поиск счёта или банка..."
            className="w-full bg-[#F3F4F6] dark:bg-[#20222A] text-gray-900 dark:text-white rounded-2xl pl-10 pr-9 py-2.5 text-[14px] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#2B5BFF] transition-all"
          />
          {searchQuery && (
            <button
              type="button"
            ...
            >
              <CloseCircleFilled className="text-[15px]" />
            </button>
          )}
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center space-x-3.5 min-w-0 flex-1 mr-2">
                    <div
                      className="w-12 h-12 rounded-[16px] flex items-center justify-center text-2xl shadow-sm flex-shrink-0"
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center space-x-1.5 flex-wrap gap-y-0.5">
                        <h4 className="text-[15px] font-semibold text-[#111827] dark:text-white leading-tight truncate">
                          {resolved.cleanName}
                        </h4>
                        {resolved.bank && (
